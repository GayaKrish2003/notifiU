...
exports.createEvent = async (req, res) => {
    try {
        const { title, description, date, time, location, organizingClub, category } = req.body;

        // Explicit Validation
        if (!title || !description || !date || !time || !location || !organizingClub || !category) {
            return res.status(400).json({ message: "All required fields (title, description, date, time, location, organizingClub, category) must be provided" });
        }
