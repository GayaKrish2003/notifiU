...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      // Save full user data so dashboards can read studentId, name, phonenumber etc.
      ...
      if (data.role === "superadmin") {
        ...
        navigate("/clubpresident-dashboard");
      } else {
        navigate("/dashboard");
      }
    } catch (err: unknown) {
      showError(
        "Login Failed",
        (err as ApiError).response?.data?.message || "Please check your credentials and try again."
      );
    } finally {
      setLoading(false);
    }
