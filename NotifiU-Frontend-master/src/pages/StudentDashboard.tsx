...
type TabKey = "home" | "module" | "events" | "faqs" | "profile" | "jobs";
...
const StudentDashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (activeTab === "jobs" && jobView === "browse") {
      fetchJobs();
      fetchSavedJobs(); // pre-load bookmarked IDs so buttons show correct state
      fetchAppliedJobs(); // pre-load applied IDs
    }
  }, [activeTab]);
  ...
  // ─── Jobs state ──────────────────────────────────────────────────────────────
  type JobView = "browse" | "saved" | "applied";
  const [jobView, setJobView] = useState<JobView>("browse");
  ...
  const fetchJobs = (search?: string, jobType?: string): void => {
    setJobsLoading(true);
    const params: Record<string, string> = {};
    if (search) params.search = search;
    if (jobType) params.jobType = jobType;
    getApprovedJobPosts(params)
      .then((res) => setJobPosts(res.data.data))
      .catch(() => setJobPosts([]))
      .finally(() => setJobsLoading(false));
  };

  const fetchSavedJobs = (): void => {
    getBookmarkedJobs()
      .then((res) => {
        setSavedJobs(res.data.data);
        // Build a Set of bookmarked IDs for fast lookup
        setBookmarkedIds(new Set(res.data.data.map((j: JobPost) => j._id)));
      })
      .catch(() => setSavedJobs([]));
  };

  const fetchAppliedJobs = (): void => {
    getAppliedJobs()
      .then((res) => {
        setAppliedJobs(res.data.data);
        // Build a Set of applied IDs for fast lookup
        setAppliedIds(new Set(res.data.data.map((j: JobPost) => j._id)));
      })
      .catch(() => setAppliedJobs([]));
  };

  const handleToggleBookmark = (jobId: string): void => {
  ...
  const handleToggleApplied = (jobId: string): void => {
    toggleMarkApplied(jobId)
      .then(() => {
        setAppliedIds((prev) => {
          const next = new Set(prev);
          if (next.has(jobId)) {
      ...
      .catch(() => alert("Failed to update applied status."));
  };

  const handleApplyNow = (job: JobPost): void => {
    // Increment view count silently — don't wait for it
    incrementViewCount(job._id).catch(() => {});

    // Make sure the URL has a protocol (http:// or https://)
    // If someone saved "Youtube.com" without https://, we add it automatically
    // Without this, the browser treats it as a relative path like /Youtube.com
      ...
      <main className="max-w-[1600px] mx-auto px-16 py-16">
            ...
            {faqView === "faq" && (
              <div>
                <div className="flex justify-between items-center mb-10">
                  <h1 className="text-2xl font-bold text-[#2D3A5D]">FAQ</h1>
        ...
        {activeTab === "jobs" && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="fixed inset-0 bg-black/30 z-[100] flex items-center justify-center p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
                  ...
                  <div className="px-8 py-6 space-y-5">
                    <div>
                      <h4 className="text-[#2D3A5D] font-black text-xl mb-1">
                        {selectedJob.title}
                      </h4>
                      <p className="text-[#2D3A5D]/50 text-sm font-bold capitalize">
                        {selectedJob.companyName} · {selectedJob.jobType} ·{" "}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        ...
                        {
                          label: "Deadline",
                          value: formatJobDate(selectedJob.deadline),
                        },
                      ].map(({ label, value }) => (
                        <div key={label}>
                    ...
                    <div className="flex gap-3 pt-2">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-2 bg-[#FBB017] hover:bg-[#e9a215] text-white font-black text-xs uppercase tracking-widest py-3 rounded-xl transition-all shadow-lg"
                      >
                        <ExternalLink size={14} />
                        Apply Now
                      </button>
                      <button
                        onClick={() => handleToggleBookmark(selectedJob._id)}
                        className={`p-3 rounded-xl border transition-all ${
                          bookmarkedIds.has(selectedJob._id)
                            ? "bg-[#FBB017] border-[#FBB017] text-white"
                            : "border-gray-200 text-gray-400 hover:border-[#FBB017] hover:text-[#FBB017]"
                        }`}
                      >
                        <Bookmark size={16} />
                      </button>
                      <button
                        onClick={() => handleToggleApplied(selectedJob._id)}
                        className={`p-3 rounded-xl border transition-all ${
                          appliedIds.has(selectedJob._id)
                            ? "bg-green-500 border-green-500 text-white"
                            : "border-gray-200 text-gray-400 hover:border-green-500 hover:text-green-500"
                        }`}
            ...
            {/* ── Top bar: view switcher + search + filter ── */}
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
              ...
              {/* View switcher */}
              <div className="flex items-center gap-2 bg-[#EBECEF]/60 rounded-2xl p-1">
                {(["browse", "saved", "applied"] as JobView[]).map((v) => (
                  <button
                    key={v}
                    onClick={() => {
                      setJobView(v);
                      if (v === "browse") fetchJobs();
                      if (v === "saved") fetchSavedJobs();
                      if (v === "applied") fetchAppliedJobs();
                    }}
            ...
            {/* ── Search + Filter (only on browse view) ── */}
            {jobView === "browse" && (
              <div className="flex flex-col lg:flex-row gap-3 mb-8">
                <div className="relative flex-1 group">
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter")
                        fetchJobs(jobSearch, jobTypeFilter);
                    }}
                    placeholder="Search by title, company or skill..."
                    className="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 pl-14 pr-6 text-sm outline-none focus:bg-white focus:border-[#FBB017]/30 text-[#2D3A5D] font-medium transition-all"
                  />
                </div>
                <select
                  value={jobTypeFilter}
                  onChange={(e) => {
                    ...
                    fetchJobs(jobSearch, e.target.value);
                  }}
                  className="bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 text-sm text-[#2D3A5D] font-medium outline-none focus:border-[#FBB017]/30 transition-all appearance-none cursor-pointer lg:w-48"
                >
                ...
                <button
                  onClick={() => fetchJobs(jobSearch, jobTypeFilter)}
                  className="bg-[#FBB017] hover:bg-[#e9a215] text-white font-black text-xs uppercase tracking-widest px-8 py-4 rounded-2xl transition-all shadow-md"
                >
                  Search
                </button>
              </div>
            ...
            {/* Browse view */}
            {!jobsLoading && jobView === "browse" && (
              <div className="space-y-4">
                {jobPosts.length === 0 ? (
                  <p className="text-center text-[#2D3A5D]/20 font-bold tracking-widest py-20">
                    No job posts found.
                  </p>
                ) : (
                  jobPosts.map((job) => (
                    <div
                      key={job._id}
                      className="bg-[#EBECEF]/40 hover:bg-white border border-transparent hover:border-[#FBB017]/10 rounded-2xl px-8 py-6 transition-all hover:shadow-md"
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3 flex-wrap mb-2">
                            ...
                            <span className="bg-[#FBB017]/10 text-[#2D3A5D] text-[10px] font-black px-3 py-1 rounded-full capitalize">
                              {job.jobType}
                            </span>
                          ...
                          <div className="flex items-center gap-4 text-[#2D3A5D]/40 text-xs font-bold mb-3">
                            ...
                            <span className="flex items-center gap-1">
                              <Clock size={11} />
                              Deadline: {formatJobDate(job.deadline)}
                            </span>
                          </div>
                          {job.skills.length > 0 && (
                            <div className="flex flex-wrap gap-1.5">
                              {job.skills.slice(0, 4).map((skill) => (
                                <span
                                  key={skill}
                                  className="bg-white border border-gray-100 text-[#2D3A5D]/60 text-[10px] font-bold px-2.5 py-1 rounded-lg"
                                >
                                  {skill}
                                </span>
                              ))}
                              {job.skills.length > 4 && (
                                <span className="text-[#2D3A5D]/30 text-[10px] font-bold px-2 py-1">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            ...
                            className="bg-[#FBB017] hover:bg-[#e9a215] text-[#2D3A5D] text-[9px] font-black px-4 py-2 rounded-xl transition-all uppercase tracking-widest shadow-sm"
                          >
                            View
                          </button>
                          <button
                            onClick={() => handleApplyNow(job)}
                            className="flex items-center gap-1 border border-[#2D3A5D]/10 hover:border-[#FBB017] text-[#2D3A5D]/50 hover:text-[#FBB017] text-[9px] font-black px-4 py-2 rounded-xl transition-all uppercase tracking-widest"
                          >
                          ...
                          <button
                            ...
                            className={`p-2 rounded-xl border transition-all ${
                              bookmarkedIds.has(job._id)
                                ? "bg-[#FBB017] border-[#FBB017] text-white"
                                : "border-gray-200 text-gray-300 hover:border-[#FBB017] hover:text-[#FBB017]"
                            }`}
                          >
                            <Bookmark size={13} />
                          </button>
            ...
            {/* Saved jobs view */}
            {!jobsLoading && jobView === "saved" && (
              <div className="space-y-4">
                {savedJobs.length === 0 ? (
                  <p className="text-center text-[#2D3A5D]/20 font-bold tracking-widest py-20">
                    No saved jobs yet. Click the bookmark icon on any job.
                  </p>
                ) : (
                  savedJobs.map((job) => (
                    <div
                      key={job._id}
                      className="bg-[#EBECEF]/40 hover:bg-white border border-transparent hover:border-[#FBB017]/10 rounded-2xl px-8 py-5 transition-all hover:shadow-md flex items-center justify-between gap-4"
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-[#2D3A5D] font-black text-sm uppercase tracking-wide mb-1">
                          {job.title}
                        </p>
                        <p className="text-[#2D3A5D]/40 font-bold text-xs capitalize">
                          {job.companyName} · {job.jobType} · Deadline:{" "}
                      ...
                      <div className="flex gap-2 shrink-0">
                        <button
                          onClick={() => setSelectedJob(job)}
                          className="bg-[#FBB017] hover:bg-[#e9a215] text-[#2D3A5D] text-[9px] font-black px-4 py-2 rounded-xl transition-all uppercase tracking-widest"
                        >
                          View
                        </button>
                        <button
                          ...
                          className="border border-gray-200 text-gray-400 hover:border-red-300 hover:text-red-400 text-[9px] font-black px-4 py-2 rounded-xl transition-all uppercase tracking-widest"
                        >
                          Remove
                        </button>
                      </div>
            ...
            {/* Applied jobs view */}
            {!jobsLoading && jobView === "applied" && (
              <div className="space-y-4">
                {appliedJobs.length === 0 ? (
                  <p className="text-center text-[#2D3A5D]/20 font-bold tracking-widest py-20">
                    No applied jobs yet. Mark jobs as applied after you submit
                    your application.
                  </p>
                ) : (
                  appliedJobs.map((job) => (
                    <div
                      key={job._id}
                      className="bg-[#EBECEF]/40 hover:bg-white border border-transparent hover:border-green-200 rounded-2xl px-8 py-5 transition-all hover:shadow-md flex items-center justify-between gap-4"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <p className="text-[#2D3A5D] font-black text-sm uppercase tracking-wide">
                            {job.title}
                          </p>
                          <span className="bg-green-100 text-green-700 text-[10px] font-black px-3 py-0.5 rounded-full">
                            ✓ Applied
                          </span>
                        </div>
                        <p className="text-[#2D3A5D]/40 font-bold text-xs capitalize">
                      ...
                      <button
                        onClick={() => setSelectedJob(job)}
                        className="bg-[#FBB017] hover:bg-[#e9a215] text-[#2D3A5D] text-[9px] font-black px-4 py-2 rounded-xl transition-all uppercase tracking-widest shrink-0"
                      >
                        View
                      </button>
                    </div>
        ...
        {activeTab === "module" && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
            {selectedModuleId ? (
              ...
              <StudentModules onOpenModule={setSelectedModuleId} />
            )}
          </div>
        )}
      </main>
